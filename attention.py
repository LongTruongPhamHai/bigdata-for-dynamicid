...
class SAAProcessor(nn.Module):
    ...
    def compute_activate_weight(self, score, bboxs):
        ...
        if self.root_img_width is None or self.root_img_height is None:
            ...
            ratio = int(
                math.sqrt(self.root_img_width * self.root_img_height // query_len)
            )
        ...
        activate_weight = activate_weight.reshape(bsz, height, width) / (
            infer_scale + 1
        )
        ...
        for idx, bbox in enumerate(bboxs):
            ...
            activate_weight[:, y_min:y_max, x_min:x_max] = (
                activate_weight[:, y_min:y_max, x_min:x_max] * (infer_scale + 1)
                + infer_scale
            )
            for i in range(len(bboxs)):
                if i != idx:
                    score[:, :, y_min:y_max, x_min:x_max, i * 4 : (i + 1) * 4] = float(
                        "-inf"
                    )
    ...
    def __call__(
        ...
        if encoder_hidden_states is None:
            ...
            end_pos = encoder_hidden_states.shape[1] - self.num_tokens * int(
                self.id_num
            )
            # -------------------------------
        ...
        if self.activate_query:
            ...
            mask_score = score
            activate_weight = torch.ones((batch_size, query.shape[2])).to(
        ...
        ip_hidden_states = (
            ip_hidden_states.transpose(1, 2)
