...
class DynamicIDStableDiffusionPipeline(StableDiffusionPipeline):
    ...
    def load_DynamicID(
        ...
        image_encoder_path: str = "openai/clip-vit-large-patch14",
        # -------------------------------
        ...
        **kwargs,
        ...
        if IMR_path is not None:
            ...
            self.key_point_encoder = KeyPointEncoder().to(
                self.device, dtype=torch.float16
            )
            ...
            self.key_point_encoder.load_state_dict(
                torch.load(key_point_encoder_path, map_location="cpu")
            )
            ...
            checkpoint = torch.load(
                "./models/ResEmoteNet/fer2013_model.pth", weights_only=True
            )
    ...
    def get_image_embeds(self, faceid_embeds, face_image, s_scale, shortcut=False):
        ...
        image_prompt_tokens = self.image_proj_model(
    ...
    def cal_face_feature(self, input_id_image):
        ...
        face_feature, uncond_image_prompt_embeds = self.get_image_embeds(
            faceid_embeds, face_image=input_id_image, s_scale=1.0, shortcut=False
        )
    ...
    @torch.no_grad()
    def get_edited_token(
        self,
        src_img,
        tgt_face_prompt=None,
        tgt_landmark_path=None,
        src_face_prompt=None,
    ):
        ...
        src_landmark = (
            self.landmark_transform(src_landmark_img)
            .unsqueeze(0)
            .to(self.device, dtype=torch.float16)
        )
        tgt_landmark = (
            self.landmark_transform(tgt_landmark_img)
            .unsqueeze(0)
            .to(self.device, dtype=torch.float16)
        )
        ...
        txt_embeds = (
            self.text_encoder(text_inputs.input_ids.to(self.device))[1]
            .detach()
        ...
        edited_feature = self.IMR(
            src_token, src_lmk_ft, tgt_lmk_ft, src_txt_embeds, tgt_txt_embeds
        )
    ...
    def __call__(
        ...
        uncond_image_prompt_embeds = torch.cat(
            [self.uncond_image_prompt_embeds] * id_num, dim=1
        )
