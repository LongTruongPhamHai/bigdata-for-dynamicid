...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, root_path="./dataset/", standard_size=32):
        ...
        for e in exp:
            for p in pose:
                self.face_prompts.append(
                    "the expression is " + e + ", the face is " + p
                )

    def get_valid_data_id(self, idx, ids):
        id = random.sample(ids, 1)[0]
        landmark_path = os.path.join(
            self.image_root_path, str(idx), id + "_landmark.png"
        )
    ...
    def __getitem__(self, idx):
        ...
        source_token = torch.load(
            os.path.join(self.token_root_path, str(idx), source_id + ".pt"),
            map_location="cpu",
        )
        source_face_prompt_path = os.path.join(
            self.image_root_path, str(idx), source_id + ".txt"
        )
        with open(source_face_prompt_path, "r") as f:
            source_face_prompt = f.read()
        source_landmark = Image.open(
            os.path.join(self.image_root_path, str(idx), source_id + "_landmark.png")
        ).convert("L")
        ...
        for i in range(self.standard_size):
            ...
            target_token = torch.load(
                os.path.join(self.token_root_path, str(idx), id + ".pt"),
                map_location="cpu",
            )
            # ------------- FIX -------------
            # raw_image = Image.open(
            #     os.path.join(self.image_root_path, str(idx), id + ".png")
            # )
            raw_image = Image.open(
                os.path.join(self.image_root_path, str(idx), id + ".jpg")
            )
            # -------------------------------

            target_face_prompt_path = os.path.join(
                self.image_root_path, str(idx), id + ".txt"
            )
            landmark_image = Image.open(
                os.path.join(self.image_root_path, str(idx), id + "_landmark.png")
            ).convert("L")
            landmark_image = self.transform(landmark_image)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help=("The depth of an IMR network."),
    )
    parser.add_argument(
        "--num_tokens",
        type=int,
    ...
    for epoch in range(num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with torch.no_grad():
                ...
                latents = vae.encode(
                    target_images.to(device, dtype=weight_dtype)
                ).latent_dist.sample()
            ...
            pred_tokens = imr(
                source_token,
                source_landmark_feature,
                target_landmark_feature,
                src_face_prompt,
                tgt_face_prompt,
            )

            tar_noise_pred = DynamicID_model(
                noisy_latents, timesteps, prompt_embeds, target_tokens
            )
            noise_pred = DynamicID_model(
                noisy_latents, timesteps, prompt_embeds, pred_tokens
            )

            DFM_loss = F.mse_loss(
                pred_tokens.float(), target_tokens.float(), reduction="mean"
            )
            LDC_loss = F.mse_loss(
                noise_pred.float(), tar_noise_pred.float(), reduction="mean"
            )
            ...
            if (step + 1) % save_steps == 0:
                if accelerator.is_main_process:
                    print(
                        "Epoch {}, step {}, DFM_loss_sum: {}, LDC_loss_sum: {}".format(
                ...
                # ------------- FIX -------------
                os.makedirs("./models/save_model", exist_ok=True)
                # -------------------------------

                torch.save(
                    imr.state_dict(), f"./models/save_model/IMR_{epoch+1}_{step+1}.bin"
                )
                torch.save(
                    keypoint_encoder.state_dict(),
                    f"./models/save_model/keypoint_encoder_{epoch+1}_{step+1}.bin",
                )
