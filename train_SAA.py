...
class IPAdapter(torch.nn.Module):
    ...
    def load_from_checkpoint(self, ckpt_path: str):
        ...
        if (
            "latents" in state_dict["image_proj"]
            and "latents" in self.image_proj_model.state_dict()
        ):
            # Check if the shapes are mismatched
            if (
                state_dict["image_proj"]["latents"].shape
                != self.image_proj_model.state_dict()["latents"].shape
            ):
                print(
                    f"Shapes of 'image_proj.latents' in checkpoint {ckpt_path} and current model do not match."
                )
                print(
        ...
        self.image_proj_model.load_state_dict(
            state_dict["image_proj"], strict=strict_load_image_proj_model
        )
...
def main():
    ...
    for name in unet.attn_processors.keys():
        ...
        if cross_attention_dim is None:
            attn_procs[name] = SAProcessor(hidden_size=hidden_size)
    ...
    for epoch in range(0, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(ip_adapter):
                ...
                clip_images = []
                for clip_image, drop_image_embed in zip(
                    batch["clip_images"], batch["drop_image_embeds"]
                ):
                ...
                with torch.no_grad():
                    image_embeds = image_encoder(
                        clip_images.to(accelerator.device, dtype=weight_dtype),
